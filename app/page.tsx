...
function Accordion({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border border-[#1a1a1a] rounded-lg bg-[#111] overflow-hidden">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-4 py-3 text-xs font-mono text-[#14f195] tracking-widest hover:bg-[#151515] transition-colors"
      >
        <span>{label}</span>
        <span className="text-[#444] text-base leading-none">{open ? '▴' : '▾'}</span>
      </button>
        ...
        <div className={`grid gap-6 lg:gap-8 ${showRight ? 'lg:grid-cols-2' : ''}`}>
          ...
          {showRight && (
            <div className="flex flex-col gap-3">
              {loading ? (
                <div className="border border-dashed border-[#2a2a2a] rounded-lg bg-[#111] p-4 sm:p-5 flex flex-col gap-3 animate-pulse">
                  {[...Array(5)].map((_, i) => (
                ...
                <>
                  {/* Receipt card */}
                  <div className="border border-dashed border-[#2a2a2a] rounded-lg bg-[#111] p-4 sm:p-5">
                    <p className="text-[#14f195] text-xs tracking-widest font-mono mb-3 sm:mb-4">RECEIPT</p>
                    ...
                    <div className="flex justify-between items-baseline mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-[#1a1a1a]">
                  ...
                  {result.pnl && (
                    <Accordion label="REAL P&amp;L">
                      <p className={`text-sm font-bold font-mono mt-1 mb-3 ${result.pnl.totalNetValueUsd >= 0 ? 'text-green-400' : 'text-red-400'}`}>
